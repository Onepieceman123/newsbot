        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if resp.status != 200:
                    ...
                    print(f"[NewsAPI] status={resp.status} body={text}")
                    return []
        ...
        for guild in bot.guilds:
            try:
                ...
                if not api_key or not channel_id:
                    print(f"[Bot] {guild.name}: missing api_key or channel_id")
                    continue

                elapsed_hours = (now.timestamp() - last_posted) / 3600
                if elapsed_hours < interval_hours:
                    continue

                channel = bot.get_channel(int(channel_id))
                if channel is None:
                    print(f"[Bot] {guild.name}: channel {channel_id} not found")
                    continue

                articles = await fetch_news(api_key, category)
                print(f"[Bot] {guild.name}: fetched {len(articles)} article(s)")
                ...
                for article in articles:
                    ...
                    if not url:
                        continue
                    if url in guild_seen:
                        continue
                    new_articles.append(article)

                print(f"[Bot] {guild.name}: {len(new_articles)} unseen article(s)")
                ...
                print(f"[Bot] {guild.name}: sent {sent_count} article(s)")
...
async def slash_categories(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🗂️ Available News Categories",
        description="Pick one with `/post interval` or `/setup category`:",
...
async def slash_news(interaction: discord.Interaction, category: str = None):
    ...
    if not api_key:
        await interaction.response.send_message(
            "❌ No API key set. Owner needs to run `/setup apikey` first.",
            ephemeral=True,
        )
        return
...
async def post_interval(
    ...
    if category not in VALID_CATEGORIES:
        await interaction.response.send_message(
            "❌ Invalid category. Use `/categories` to see options.",
            ephemeral=True,
        )
        return
    ...
    try:
        ...
        await interaction.response.send_message(
            "❌ Invalid time. Pick from the dropdown or type a number.",
            ephemeral=True,
        )
        return
...
async def setup_category(interaction: discord.Interaction, category: str):
    ...
    set_config(interaction.guild.id, "category", category)
    await interaction.response.send_message(
        f"{CATEGORY_EMOJIS[category]} Category updated to **{category}**!",
        ephemeral=True,
    )
...
async def setup_apikey(interaction: discord.Interaction, apikey: str):
    ...
    if not articles:
        await interaction.followup.send(
            "❌ That API key doesn't work. Get one free at https://newsapi.org/register",
            ephemeral=True,
        )
        return
...
async def setup_status(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="📢 Channel",
        value=f"<#{channel_id}>" if channel_id else "❌ Not set — use `/post interval`",
        inline=False,
    )
    embed.add_field(
        name="🗂️ Category",
        value=f"{CATEGORY_EMOJIS.get(category, '')} {category}" if category in CATEGORY_EMOJIS else "❌ Not set",
        inline=False,
    )
    embed.add_field(
        name="🔑 API Key",
        value="✅ Set" if api_key else "❌ Not set — use `/setup apikey`",
        inline=False,
    )
    embed.add_field(
        name="⏱️ Interval",
        value=f"Every {format_hours(interval_hours)}",
        inline=False,
    )
    ...
    for channel in guild.text_channels:
        if channel.permissions_for(me).send_messages:
            embed = discord.Embed(
                ...
                description=(
                    "Quick setup — the **server owner** runs just 2 commands:\n\n"
                    "1️⃣ `/setup apikey YOUR_KEY` — free key at https://newsapi.org/register\n"
                    "2️⃣ `/post interval` — pick your channel, category & frequency\n\n"
                    "📋 `/help` — see all commands\n"
                    "🗂️ `/categories` — see all news topics"
                ),
                color=discord.Color.blurple(),
            )
