...
async def slash_help(interaction: discord.Interaction):
    ...
    embed.add_field(name="👤 Everyone", value="`/help` — This menu\n`/news` — Get latest news now\n`/categories` — See all news topics", inline=False)
    embed.add_field(name="👑 Owner Only", value="`/setup apikey` — Set your NewsAPI key\n`/setup category` — Change news topic\n`/setup status` — Check current settings\n`/post interval` — Set channel, category & frequency", inline=False)
