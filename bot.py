...
async def slash_help(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`/post interval` — Set channel, category & frequency all at once"
        ),
        inline=False
    )
...
@post_group.command(name="interval", description="Set the channel, category and how often news is posted — all in one")
...
async def post_interval(
    ...
    try:
        ...
        await interaction.response.send_message("❌ Invalid time. Pick from the dropdown or type a number like `6`.", ephemeral=True)
        return
    ...
    await interaction.response.send_message(
        ...
        f"⏱️ Posting every **{format_hours(hours_float)}**",
        ephemeral=True
    )
